...
tyrannosaurus.roar = () => "rawrarwaarawwr!"
...
const universities = []; {
  for (let i = 0; i < graduates.length; i++){
    universities.push(graduates[i].univeristy);
  }
  universities.sort();
}
...
const contactInfo = []; {
  for (let i = 0; i <graduates.length; i++){
    contactInfo.push(`${graduates[i].first_name} ${graduates[i].eail}`);
  }
}
...
const lowerPopulation = zooAnimals.filter(arrItems => arrItems.population < 5);
